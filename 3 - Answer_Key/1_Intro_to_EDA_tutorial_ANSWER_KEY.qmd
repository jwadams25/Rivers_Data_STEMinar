---
title: "Tutorial 1: Introduction to EDA - ANSWER KEY"
author: "ANSWER KEY"
editor: visual
format:
  gfm:
...
**ANSWER: The data was taken from Lending Club.**
:::

::: {style="color: black; background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
5.  What does that platform do?

**ANSWER: Lending Club is a peer-to-peer lending platform that connects borrowers with investors. Borrowers apply for loans and investors can choose to fund those loans.**
:::

::: {style="color: black; background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
6.  What are the observational units in the `loans50` dataset

**ANSWER: The observational units are individual loans (each row represents one loan made through Lending Club).**
:::

::: {style="color: black; background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
7.  How many observations are there?

**ANSWER: There are 50 observations (loans).**
:::

::: {style="color: black; background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
8.  How many variables are there?

**ANSWER: There are 18 variables.**
:::
...
**ANSWER: We also narrowed the data frame from 18 variables down to 9 variables by using the `select` function to keep only the columns we need: loan_amount, interest_rate, term, grade, state, total_income, homeownership, loan_purpose, and total_credit_limit. We also saved the result to a new object called `loans` using the assignment operator (`<-`).**
:::
...
loans |>
  ...
  labs(
    x = "Loan Amount ($)",
    y = "Count",
    title = "Distribution of Loan Amounts by Homeownership Status"
  ) +
  theme_minimal() +
...
**ANSWER: loan_amount_hat = intercept + slope × total_income**

*Note: The specific numerical values for the intercept and slope will be generated when the code chunk above is run. Students should write the equation using those values, e.g.: loan_amount_hat = b0 + b1 × total_income (where b0 is the intercept and b1 is the slope from the output).*
:::
...
**ANSWER: The slope coefficient is positive, which indicates a positive association between total income and loan amount. This means that as total income increases, the loan amount also tends to increase. For every one dollar increase in total income, the predicted loan amount increases by the slope value.**
:::

::: {style="color: black; background-color: white; padding: 15px; border: 5px solid #006CB3; border-radius: 5px;"}
7.  Use the R-squared value to describe the strength of the association.

**ANSWER: The R-squared value tells us the proportion of the variability in loan amount that is explained by the linear model using total income as a predictor. For example, if R-squared = 0.10, that means about 10% of the variability in loan amounts can be explained by total income. A low R-squared suggests a weak association, while a higher R-squared would indicate a stronger association. Students should insert the actual R-squared value from their output and interpret it accordingly.**
:::
