...
```{r select}
loans <- loan50 |>
          SELECT_FUCTION(loan_amount, interest_rate, term, grade, 
...
NAME_OF_DATA_FRAME |>
  ...
  summarise(
    mean(NUMERICAL_VARIABLE),
    median(NUMERICAL_VARIABLE),
    sd(NUMERICAL_VARIABLE),
    quantile(NUMERICAL_VARIABLE, 0.25),
    quantile(NUMERICAL_VARIABLE, 0.75),
  )
```
...
loan |>
  ggplot(aes(x = , y = )) +
  geom_point() +
  geom_smooth(method = TYPE_OF_MODEL, se = FALSE) +
  labs(
    x = "", 
    y = "",
    title = ""
...
```{r the model}
lm(data = NAME_OF_DATA_FRAME, formula = OUTCOME~PREDICTOR)
```
